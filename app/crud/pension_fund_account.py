...
from app.schemas.pension_fund_account import PensionFundAccountCreate, PensionFundAccountUpdate

def get_pension_fund_accounts(db: Session, skip: int = 0, limit: int = 100):
    ...
    try:
        accounts = db.query(
            ...
            PensionFundAccount.financial_institution_code == FinancialInstitution.code
        ).offset(skip).limit(limit).all()
        
        # 결과를 PensionFundAccount 객체에 금융기관명 추가
        result = []
        ...
        accounts = db.query(PensionFundAccount).offset(skip).limit(limit).all()
        for account in accounts:
            account.financial_institution_name = None
        return accounts

def get_pension_fund_account(db: Session, account_id: int):
    try:
        account = db.query(
            PensionFundAccount,
            FinancialInstitution.name.label('financial_institution_name')
            ...
            FinancialInstitution,
            PensionFundAccount.financial_institution_code == FinancialInstitution.code
        ).filter(PensionFundAccount.id == account_id).first()
        ...
        # 테이블 구조가 아직 업데이트되지 않은 경우, 기존 방식으로 조회
        account_obj = db.query(PensionFundAccount).filter(PensionFundAccount.id == account_id).first()
        if account_obj:
            account_obj.financial_institution_name = None
        return account_obj

def create_pension_fund_account(db: Session, account: PensionFundAccountCreate):
    db_account = PensionFundAccount(**account.model_dump())
    db.add(db_account)
    db.commit()
...
def update_pension_fund_account(db: Session, account_id: int, account: PensionFundAccountUpdate):
    try:
        db_account = get_pension_fund_account(db, account_id)
    except Exception as e:
        # 테이블 구조가 아직 업데이트되지 않은 경우, 직접 조회
        db_account = db.query(PensionFundAccount).filter(PensionFundAccount.id == account_id).first()
    
    if not db_account:
        raise ValueError(f"연금저축펀드 계좌를 찾을 수 없습니다: {account_id}")
...
def delete_pension_fund_account(db: Session, account_id: int):
    db_account = db.query(PensionFundAccount).filter(PensionFundAccount.id == account_id).first()
    if not db_account:
        raise ValueError(f"연금저축펀드 계좌를 찾을 수 없습니다: {account_id}")
